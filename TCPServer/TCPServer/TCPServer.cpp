...
int TCPServer::addPost(const PostRequest &pr) { //add a new post to posts.
	std::unique_lock<std::mutex> postLock(postMutex);
	Post p = { posts[pr.topicId].size(), pr.message }; //create post

	if (p.text.length() > 140) //truncate string
		p.text = p.text.substr(0, 140);
	
	posts[pr.topicId].push_back(p); //add post to posts via topicId
	postLock.unlock();
	return p.id;
}

std::string TCPServer::readPost(const ReadRequest& rr) {
	//return an empty line if topic doesnt exist or postID is too large
	if (!(posts.count(rr.topicId) && posts[rr.topicId].size() - 1 >= rr.postId))
		return "\n";

	Post post = posts[rr.topicId][rr.postId];
	return post.text;
}

int TCPServer::countPosts(const CountRequest& cr) {
	
	if (posts.count(cr.topicId))
		return  posts[cr.topicId].size();
	else
		return 0;
...
std::string TCPServer::List(const ListRequest& list) {
	std::string s("");
	for (std::map <std::string , std::vector<Post>> ::iterator it = posts.begin(); it != posts.end(); ++it) {
		s += it->first + "#";	
	}
